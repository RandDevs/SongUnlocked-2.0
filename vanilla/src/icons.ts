/**
 * Hand-drawn stroke icons on a 24×24 grid.
 * No icon font, no emoji, no dependency.
 */
...
const SHAPES: Record<string, Shape> = {
  ...
  // A tuning fork: musical, and not the usual guitar cliche.
  ...
  // Mood icons. Drawn on the same 24x24 grid at the same stroke weight as the
  // rest of the set, so a mood chip never looks pasted in from another app.
  moodSad: {
    d: [
  ...
  // A cassette: the shortest visual route to "this song is from before".
  moodNostalgic: {
    d: [
      "M3.5 6.5h17v11h-17z",
...
export function icon(name: string, size: number = 20): SVGSVGElement {
